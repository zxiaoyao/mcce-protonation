 ...
 * Premcce
 *
 * This module is the step 1 of mcce. It does these:
...
int output_sd_pdb(PROT prot);

int premcce()
{
	FILE *fp1, *fp2, *fp;
	char sbuff[MAXCHAR_LINE];		// string buffer to read a line of a file
	PROT prot;
	int Counter, Nhoh;
	int i_res, i_conf;
	...
	/* copy the content in fp1 to fp2 */
	fp2 = tmpfile();
	/* original code, which read the entire pdb file
    while ((c=fgetc(fp1)) != EOF) fputc(c, fp2);
    fputc(EOF, fp2);
	 */
	while (fgets(sbuff, sizeof(sbuff), fp1)) {
		// a pdb line should be longer than 50 characters
		if (strlen(sbuff) < 50) continue;

		// only read in pdb lines starting with "ATOM  " or "HEATOM"
	...
	printf("   Done\n\n");
	/* pdb now in fp2 */
	/* fp1 points to the name.txt file */
	...
	printf("   Done\n\n");
	fflush(stdout);
	/* pdb now in fp1 again, with the renamed pdb lines */
	...
	/* pdb now in fp2 again, with possible terminal residues labeled */
	...
	while (!(fp1 = premcce_confname(fp2))) {
		if (env.err_msg >= 0) {
			...
			if (create_param(fp2, env.err_msg)) {
				printf("   parameter file %s not made.\n",env.new_tpl);
				exit(-1);
			}
...
//	fclose(fp);
	...
	// Output a standard pdb file for delphi, the same as "step1_out.dpb" except format.
	output_sd_pdb(prot);
...
//	write_headlst(prot);  /* write out */
	...
	nowEnd = time(NULL);
	printf("\n Total time for step1 (premcce) is %ld seconds.\n\n", nowEnd - nowStart);
...
/**
 * Rename the residue if it's at terminal,
 *  by searching through all the " N  " and " C  ", and calculating the distance between them.
 * If there is no N-C pair of a distance within certain range, the residue is a terminal.
 * @param fp_in original input fp.
 * @return fp after renaming the terminals.
 */
FILE *premcce_terminals(FILE *fp_in)
{
	ATOM *Ns, *Cs;
...
FILE *premcce_confname(FILE *fp)
{
	...
	env.err_msg = -1;  // "err_msg" shows the index of pdb line where error occors

	while (fgets(sbuffer, MAXCHAR_LINE, fp)) {
		pdb.n++;
		pdb.strings = (char **)realloc(pdb.strings, pdb.n*sizeof(char *));
		...
		if (!strncmp("ATOM  ", sbuffer, 6) || !strncmp("HETATM", sbuffer, 6)) {
			...
			if (sbuffer[16] != ' ') {           // sbuffer[16] is the character right after the atom name
				sbuffer[27] = '_';
				sbuffer[28] = '_';
		...
		if (!strncmp("ATOM  ", sbuffer, 6) || !strncmp("HETATM", sbuffer, 6)) {
			...
			// get all the conformers, with the keyword "CONFLIST"
			...
			// the first conformer listed has to be the backbone, which ends with the last two letters "BK"
			...
			if (strncmp(sbuffer+80,"__",2) ) continue;         /* not "__", Conformer is given, checking the first two letters of the conformer history */

			// this atom is in the backbone, by checking the "IATOM" entry of the tpl file
			...
			if (conflist.n>1) {
				// atom found in the first conformer, also by checking the "IATOM" entry of the tpl file


				if (!param_get("IATOM", conflist.strings[1], atom.name, &iatom)) continue;


				// if can't find the atom neither in the backbone nor the first conformer, by checking the "IATOM" entry of the tpl file
				//	add it to the check list
				checklist.n++;
				checklist.strings = (char **) realloc(checklist.strings, checklist.n*sizeof(void *));
	...
	while (checklist.n) {
		...
		for (i_pdb = 0; i_pdb<pdb.n; i_pdb++) {
			if (!strncmp(pdb.strings[i_pdb]+16, checklist.strings[0]+16, 4) &&          // residue name
					!strncmp(pdb.strings[i_pdb]+21, checklist.strings[0]+21, 8) &&			// chainID and residue sequence
					!strncmp(pdb.strings[i_pdb]+80, checklist.strings[0]+80, 2) ) {			// "__"

				residue.n++;
				residue.strings = (char **) realloc(residue.strings, residue.n*sizeof(void *));
		...
		for (i_check = 0; i_check<checklist.n; i_check++) {
			if (!strncmp(checklist.strings[i_check]+16, residue.strings[0]+16, 4) &&
		...
		for (i_conf=1; i_conf<conflist.n; i_conf++) {
			...
			if (k_atom == residue.n) {
				/* If looping over all atoms is finished without broken, means all atoms can be found by current conformer name,
				 * then break looping over conformer list
				 * */
				break;
			}
		}
	...
	fp2 = tmpfile();
	for (i_pdb=0; i_pdb<pdb.n; i_pdb++) {
		//if (!strncmp(pdb.strings[i_pdb]+17, "Ub6", 3)) puts(pdb.strings[i_pdb]);
		fprintf(fp2, "%s\n",pdb.strings[i_pdb]);
	}
	rewind(fp2);
...
int create_param(FILE *pdb_fp, int k_line) {
	...
	while ( fgets(sbuffer, MAXCHAR_LINE, pdb_fp) ) {
		...
		if (!strncmp(sbuffer+16, line+16, 4) &&
			...
			conf.n_atom++;
			conf.atom = (ATOM *) realloc(conf.atom, conf.n_atom*sizeof(ATOM));
...
int output_sd_pdb(PROT prot) {
	const char *OUTPDB = "protonated.pdb";

	FILE *fp = fopen(OUTPDB, "w");
	int i, j, k, iConf, c;
	c = 0;
	for (i=0; i<prot.n_res; i++) {
		iConf =0;
		for (j=0; j<prot.res[i].n_conf; j++) {
			for (k=0; k<prot.res[i].conf[j].n_atom; k++) {
				if (!(prot.res[i].conf[j].atom[k].on)) continue;
				if (c<99999) c++;
				if (prot.res[i].chainID == '_') prot.res[i].chainID = ' ';
//				fprintf(fp, "ATOM  %5d %4s%c%3s %c%4d%c%03d%8.3f%8.3f%8.3f %7.3f      %6.3f      %-11s\n",
				fprintf(fp, "ATOM  %5d %4s%c%3s %c%4d%12.3f%8.3f%8.3f\n",
